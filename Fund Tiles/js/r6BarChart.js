...
// ******************************* ******************************* //
let isMopR = false;
function toggleSliderR6() {
    const slider = document.getElementById('slider-rbar');
    const titleNav = document.getElementById('mop-title-3-nav')
    const titleMop = document.getElementById('mop-title-3-mop')

    if (!slider.getAttribute('mop') || slider.getAttribute('mop') === 'false') {
        document.querySelector('#pill-toggle-rbar').style.left = '50%';
        document.getElementById('ft-toggle-left-r').style.color = 'rgba(127, 127, 127, 1)';
        document.getElementById('ft-toggle-right-r').style.color = 'rgba(0, 41, 75, 1)';
        isMopR = true;
...
function sliderResetR6() {
    ...
    document.querySelector('#pill-toggle-rbar').style.left = 0;
    document.getElementById('ft-toggle-left-r').style.color = 'rgba(0, 41, 75, 1)';
    document.getElementById('ft-toggle-right-r').style.color = 'rgba(127, 127, 127, 1)';
    isMopR = false;
...
class R6BarChartSection {
    ...
    initVis() {
        ...
        // CHART SVG
        vis.chartDiv1 = document.getElementById(`bar-chart-r6-1`);
        ...
        // CHART SVG
        vis.chartDiv2 = document.getElementById(`bar-chart-r6-2`);
        ...
        // CHART SVG
        vis.chartDiv3 = document.getElementById(`bar-chart-r6-3`);
        ...
        // CHART SVG
        vis.chartDiv4 = document.getElementById(`bar-chart-r6-4`);
        ...
        // CHART SVG
        vis.chartDiv5 = document.getElementById(`bar-chart-r6-5`);
        ...
        // CHART SVG
        vis.chartDiv6 = document.getElementById(`bar-chart-r6-6`);
        ...
        // CHART SVG
        vis.chartDiv7 = document.getElementById(`bar-chart-r6-7`);
        vis.chartDiv7.innerHTML = '';
        vis.svg7 = d3.select(vis.chartDiv7).append('svg').attr('class', `bar-chart-svg`);
    ...
    async redraw() {
        ...
        // added some new exceptions (if statements)
        data.forEach(el => {
            if (el.time !== "30 day SEC Yield") {
        ...
        const xTrans = seismicApp ? 30 : smallLaptop ? 30 : iPad ? 30 : 30;
        vis.yAxis
        ...
        if (data[3].value && data[3].indexValue) {
            if (document.getElementById(`bc-output-text-r6-1`)) {
                document.getElementById(`bc-output-text-r6-1`).remove();
            }
            ...
            document.getElementById(`${vis.el}-4`).innerHTML = ` 
                <div class='bar-chart-output-text' id='bc-output-text-r6-1' style='opacity: ${opacitySetting}'>
                    ${output}
                </div
            `;
            document.getElementById(`bc-output-text-r6-1`).style.top = y(0) - document.getElementById(`bc-output-text-r6-1`).clientHeight + 10 + 'px';
            document.getElementById(`bc-output-text-r6-1`).style.opacity = 1;
        }

        // chart 5
        if (data[4].value && data[4].indexValue) {
            if (document.getElementById(`bc-output-text-r6`)) {
                document.getElementById(`bc-output-text-r6`).remove();
            }
            ...
            vis.g5.append('text').text(output);
            document.getElementById(`${vis.el.replace('-main', '')}-5`).innerHTML = ` 
                <div class='bar-chart-output-text' id='bc-output-text-r6' style='opacity: ${opacitySetting}'>
                    ${output}
                </div
            `;
            document.getElementById(`bc-output-text-r6`).style.top = y(0) - document.getElementById(`bc-output-text-r6`).clientHeight + 10 + 'px';
            document.getElementById(`bc-output-text-r6`).style.opacity = 1;
        }
        // chart 6
        ...
        vis.rects7 = vis.g7.selectAll('.bars-data')
            .data([vis.data.heroData.expenses.data.find(d => d.name === 'fund').value])
        // EXIT
    ...
    setText() {
        ...
        // SET LINK
        document.getElementById('ft-launch-r6-growth').addEventListener('click', el => window.open(vis.link))

        // SET ALT ELEMENTS
        setElements('-alt');

        // DOT COLOR
        document.getElementById(`bar-chart-fund-dot-r6`).style.backgroundColor = vis.barColor;
        ...
        function setElements(group = '') {
            ...
            // FOOTNOTE FOR GROWTH
            const note = document.getElementById(`r6-growth-q-end-footnote${group}`);
            const sup = document.getElementById(`r6-bar-chart-ytd${group}`);
            ...
            if (vis.id === 'AMCAP') {
                ...
                note.innerText = '3';
                sup.innerText = '4';
            } else if (vis.id === 'WMIF' || vis.id === 'AMF') {
                note.innerText = '2';
                sup.innerText = '3';
            } else if (vis.id === 'AMBAL' || vis.id === 'GBAL') {
                note.innerText = '3';
                sup.innerText = '4';
            } else if (vis.id === 'IVE') {
                note.innerText = '2';
                ...
                note.innerText = '2';
                sup.innerText = '3';
            } else if (vis.id === 'GIF' || vis.id === 'NEF' || vis.id === 'SCWF' || vis.id === 'WGI' || vis.id === 'IGI' || vis.id === 'DWGI'
                || vis.id === 'CIB' || vis.id === 'IFA' || vis.id === 'ICA' || vis.id === 'FI') {
                note.innerText = '2';
                sup.innerText = '3';
            } else {
                note.innerText = '';
            ...
            if (vis.data.indexPrimary) {
                document.getElementById(`bar-chart-index-name-r6${group}`).innerText = vis.data.indexPrimary;
            } else {
                document.getElementById(`bar-chart-index-name-r6${group}`).innerText = '';
            }

            // DATE
            if (vis.data.barData.date) {
                document.getElementById(`bar-chart-date-r6${group}`).innerText = 'As of ' + vis.data.barData.date + ' (updated quarterly)';
            } else {
                document.getElementById(`bar-chart-date-r6${group}`).innerText = '';
            }

            // TICKER (FUND NAME)
            if (vis.data.ticker) {
                document.getElementById(`bar-chart-fund-name-r6${group}`).innerText = vis.data.ticker;
            } else {
                document.getElementById(`bar-chart-fund-name-r6${group}`).innerText = '';
            }
            ...
            data.forEach((d, i) => {
                ...
                // ROW 1 (LIGHT BLUE)
                if (d.value) {
                    document.getElementById(`bar-chart-val-${num}-r6${group}`).innerText = enDash(d.value.toFixed(2)) + '%';
                } else {
                    document.getElementById(`bar-chart-val-${num}-r6${group}`).innerText = 'N/A';
                }

                // ROW 2 (BOTTOM)
                if (d.indexValue) {
                    document.getElementById(`bar-chart-index-val-${num}-r6${group}`).innerText = enDash(d.indexValue.toFixed(2)) + '%';
                } else {
                    if (i === data.length - 1 || i === 5) {
                        document.getElementById(`bar-chart-index-val-${num}-r6${group}`).innerText = '—';
                    } else {
                        document.getElementById(`bar-chart-index-val-${num}-r6${group}`).innerText = 'N/A';
                    }
                }
            })

            // EXPENSE RATIOS
            if (document.getElementById(`bar-chart-val-7-r6${group}`)) {
                const value = vis.data.heroData.expenses.data.find(d => d.name === 'fund').value;
                document.getElementById(`bar-chart-val-7-r6${group}`).innerText = enDash(value.toFixed(2)) + '%';
            }

            // EXPENSE RATIOS
            if (document.getElementById(`bar-chart-index-val-7-r6${group}`)) {
                document.getElementById(`bar-chart-index-val-7-r6${group}`).innerText = '—';
            }
        }
    }
}