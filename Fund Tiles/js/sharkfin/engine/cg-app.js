...
document.addEventListener('keydown', event => {
    ...
    if (event.key.toLowerCase() === 'a') {
        const uiHidden =
            document.querySelector('#annotation-ui-container').style.opacity === '0' ||
            document.querySelector('#annotation-ui-container').style.right === '-45px';
...
if (document.getElementById('c-annotation')) {
    ...
    function openAnnotationUI() {
        if (panelNotMoving) {
            ...
            setTimeout(() => {
                ...
                annoui.style.boxShadow = '0px 0px 30px #00000029';
    ...
    function closeAnnotationUI() {
        ...
        if (panelNotMoving) {
            ...
            if (annoui.style.top.match(/[0-9]*/)[0] >= window.innerHeight - 563) {
                annoui.style.top = window.innerHeight - 563 + 'px';
            }

            // ELEMENT SLIDE OUT if it hasn't been repositioned by user
            if (annoui.style.right === '0px' && (annoui.style.top === '231px' || annoui.style.top === '100px')) {
                annoui.style.right = '-45px';
            } else {
                // UI ELEMENT, FADE OUT
                annoui.style.opacity = '0';
            }
            annoui.style.boxShadow = '0px 0px 30px #00000000';
    ...
    document.querySelector('#pen-color').style.backgroundColor = getPenColor();
    ...
    function handleMouseDown(e) {
        ...
        if (isHighlighting) {
            ...
            ctx.fillRect(e.offsetX, e.offsetY - 10, 25, getPenSize() === 20 ? 40 : getPenSize() === 15 ? 32 : 24);
        } else {
            let ctx = canvasAnno.getContext('2d');

            if (isErasing) {
                const brushSize = getPenSize() === 20 ? 50 : getPenSize() === 15 ? 30 : 20;
                ctx.lineCap = 'round';
    ...
    function handleMouseMove(e) {
        if (canvasAnno.down) {
            if (isHighlighting) {
                ...
                ctx.globalCompositeOperation = 'source-over';
                ctx.fillStyle = getPenColor();
                ctx.fillRect(e.offsetX, e.offsetY - 10, 28, getPenSize() === 20 ? 40 : getPenSize() === 15 ? 32 : 24);
            } else {
                let ctx = canvasAnno.getContext('2d');
                ctx.lineCap = 'round';
    ...
    function createOffset() {
        ...
        if (penSize === 20) return 20;
        else if (penSize === 15) return 15;
        else return 12;
    }
    ...
    function handleStart(e) {
        ...
        if (isHighlighting) {
            ...
            ctx.fillRect(
                ...
                28,
                getPenSize() === 20 ? 40 : getPenSize() === 15 ? 32 : 24
            );
        } else if (isErasing) {
            const brushSize = getPenSize() === 20 ? 50 : getPenSize() === 15 ? 30 : 20;
            ctx.lineWidth = brushSize;
    ...
    document.querySelectorAll('.anno-color-circle').forEach(element => {
        element.addEventListener('click', () => {
            ...
            // UPDATE UI COLOR DOT
            document.querySelector('#pen-color').style.backgroundColor = element.style.backgroundColor;
    ...
    document.getElementById('anno-eraser').addEventListener('click', () => {
        ...
        eraserIconInner.firstElementChild.style.fill = 'red';

        updateCursor();

        // TURN OFF MARKER UI AND HIGHLIGHTER UI
        document.querySelector('#anno-marker').style = '';
        document.querySelector('#highlighter-tip').style.fill = 'black';
    ...
    document.getElementById('anno-highlighter').addEventListener('click', () => {
        ...
        document.querySelector('#anno-highlighter').style.opacity = 1;
        document.querySelector('#highlighter-tip').style.fill = '#f547ef';
        ...
        document.querySelector('.eraser-holder').firstElementChild.style.fill = 'black';
    });
    ...
    // GET PEN SIZE
    function getPenSize() {
        return Number(window.localStorage.getItem('penSize')) || 15;
    }
    ...
    function draw() {
        ...
        const eraserIconUI = document.querySelector('#anno-eraser');
        eraserIconUI.style = '';
        eraserIconUI.firstElementChild.firstElementChild.firstElementChild.style.fill = 'black';
        ...
        const color = document.querySelector('#pen-color').style.backgroundColor;
    ...
    function setPen(caseVal) {
        ...
        switch (caseVal) {
            ...
            case 16:
                ...
                window.localStorage.setItem('penSize', 20);
                return 20;
            case 19:
            case 15:
            case 11:
                ...
                window.localStorage.setItem('penSize', 15);
                return 15;
            case 14:
            case 10:
            case 9:
                ...
                window.localStorage.setItem('penSize', 10);
                return 10;
            default:
    ...
    function setCursor(size) {
        ...
        if (isErasing) {
            if (size === 'big') {
                cursor.style.cursor = `url('engine/imgs/eraser-tip-big.svg') 25 25, crosshair`;
            } else if (size === 'med') {
                cursor.style.cursor = `url('engine/imgs/eraser-tip.svg') 15 15, crosshair`;
            } else {
                cursor.style.cursor = `url('engine/imgs/eraser-tip-sm.svg') 10 10, crosshair`;
            }
        } else if (isHighlighting) {
            if (size === 'big') {
                switch (color) {
                    case 'rgb(255, 255, 0)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-orange-lg.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(255, 76, 76)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-red-lg.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(255, 76, 255)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-violet-lg.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(76, 137, 253)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-blue-lg.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(52, 227, 0)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-green-lg.svg') 0 10, crosshair`;
                        return;
                    default:
                ...
                switch (color) {
                    case 'rgb(255, 255, 0)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-orange-med.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(255, 76, 76)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-red-med.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(255, 76, 255)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-violet-med.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(76, 137, 253)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-blue-med.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(52, 227, 0)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-green-med.svg') 0 10, crosshair`;
                        return;
                    default:
                        console.log('bad color');
                        cursor.style.cursor = `url('engine/imgs/highlighter-orange-med.svg') 0 10, crosshair`;
                ...
                switch (color) {
                    case 'rgb(255, 255, 0)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-orange-sm.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(255, 76, 76)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-red-sm.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(255, 76, 255)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-violet-sm.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(76, 137, 253)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-blue-sm.svg') 0 10, crosshair`;
                        return;
                    case 'rgb(52, 227, 0)':
                        cursor.style.cursor = `url('engine/imgs/highlighter-green-sm.svg') 0 10, crosshair`;
                        return;
                    default:
                        console.log('bad color');
                        cursor.style.cursor = `url('engine/imgs/highlighter-orange-sm.svg') 0 10, crosshair`;
            ...
            if (size === 'big') {
                switch (color) {
                    case 'rgb(255, 255, 0)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-20-orange.svg') 16 16, crosshair`;
                        return;
                    case 'rgb(255, 76, 76)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-20-red.svg') 16 16, crosshair`;
                        return;
                    case 'rgb(255, 76, 255)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-20-violet.svg') 16 16, crosshair`;
                        return;
                    case 'rgb(76, 137, 253)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-20-blue.svg') 16 16, crosshair`;
                        return;
                    case 'rgb(52, 227, 0)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-20-green.svg') 16 16, crosshair`;
                        return;
                    default:
                        console.log('bad color');
                        cursor.style.cursor = `url('engine/imgs/pen-tip-20-orange.svg') 16 16, crosshair`;
                ...
                switch (color) {
                    case 'rgb(255, 255, 0)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-15-orange.svg') 12 12, crosshair`;
                        return;
                    case 'rgb(255, 76, 76)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-15-red.svg') 12 12, crosshair`;
                        return;
                    case 'rgb(255, 76, 255)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-15-violet.svg') 12 12, crosshair`;
                        return;
                    case 'rgb(76, 137, 253)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-15-blue.svg') 12 12, crosshair`;
                        return;
                    case 'rgb(52, 227, 0)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-15-green.svg') 12 12, crosshair`;
                        return;
                    default:
                        console.log('bad color');
                        cursor.style.cursor = `url('engine/imgs/pen-tip-15-orange.svg') 12 12, crosshair`;
                ...
                switch (color) {
                    case 'rgb(255, 255, 0)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-10-orange.svg') 8 8, crosshair`;
                        return;
                    case 'rgb(255, 76, 76)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-10-red.svg') 8 8, crosshair`;
                        return;
                    case 'rgb(255, 76, 255)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-10-violet.svg') 8 8, crosshair`;
                        return;
                    case 'rgb(76, 137, 253)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-10-blue.svg') 8 8, crosshair`;
                        return;
                    case 'rgb(52, 227, 0)':
                        cursor.style.cursor = `url('engine/imgs/pen-tip-10-green.svg') 8 8, crosshair`;
                        return;
                    default:
                        cursor.style.cursor = `url('engine/imgs/pen-tip-10-orange.svg') 8 8, crosshair`;
                        console.log('bad color');
    ...
    function dragElement(elmnt) {
        ...
        function closeDragElement() {
            ...
            if (Number(elmnt.style.top.match(/[0-9]*/)[0]) >= window.innerHeight - 563) {
                elmnt.style.top = window.innerHeight - 563 + 'px';
            }
        }
    }
