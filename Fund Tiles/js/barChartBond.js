...
// ******************************* ******************************* //
let isMopBond = false;
function toggleSliderBondBar() {
    const slider = document.getElementById('slider-bbar');
    const titleNav = document.getElementById('mop-title-2-nav')
    const titleMop = document.getElementById('mop-title-2-mop')

    if (!slider.getAttribute('mop') || slider.getAttribute('mop') === 'false') {
        document.querySelector('#pill-toggle-bbar').style.left = '50%';
        document.getElementById('ft-toggle-left-b').style.color = 'rgba(127, 127, 127, 1)';
        document.getElementById('ft-toggle-right-b').style.color = 'rgba(0, 41, 75, 1)';
        isMopBond = true;
...
function sliderResetBondBar() {
    ...
    document.querySelector('#pill-toggle-bbar').style.left = 0;
    document.getElementById('ft-toggle-left-b').style.color = 'rgba(0, 41, 75, 1)';
    document.getElementById('ft-toggle-right-b').style.color = 'rgba(127, 127, 127, 1)';
    isMopBond = false;
    slider.setAttribute('mop', false)
}

// ******************************* ******************************* //
// MAKES BAR CHART FOR SECTION 2 BOND SECTION //
// ******************************* ******************************* //
...
class BarChartBond {
    constructor(data, fundClass, objective = 'Growth') {
        ...
        vis.el = `bar-chart-bond`;
        vis.fundClass = fundClass;
        vis.isMopBond = isMopBond;
        vis.section = 'bond';
    ...
    initVis() {
        ...
        // // CHART SVG
        // vis.chartDiv8 = document.getElementById(`${vis.el}-8`);
        // vis.svg8 = d3.select(vis.chartDiv8).append('svg').attr('class', `${vis.gID} bar-chart-svg`);
        // vis.g8 = vis.svg8.append('g').attr('class', 'g-class');
    ...
    async redraw(fundClass = this.fundClass, objective = this.objective) {
        ...
        if (vis.fundClass === 'a') {
            ...
            vis.expense = vis.shareData.a.heroData.expenseData;
        } else if (vis.fundClass === 'f2') {
            vis.data = vis.shareData.f2;
            vis.expense = vis.shareData.f2.heroData.expenseData;
        } else if (vis.fundClass === 'r6') {
            vis.data = vis.shareData.r6;
            vis.expense = vis.shareData.r6.heroData.expenseData;
        }

        // console.log('expseons', vis.expense)

        if (vis.id === 'TEBF' && vis.fundClass === 'a') {
            if (document.getElementById('tebf-sup'))
                document.getElementById('tebf-sup').style.opacity = 1;
        } else {
            if (document.getElementById('tebf-sup'))
                document.getElementById('tebf-sup').style.opacity = 0;
        }
        ...
        // vis.width8 = vis.chartDiv8.clientWidth;
        // vis.height8 = vis.chartDiv8.clientHeight;
        // vis.gWIDTH8 = vis.width8 - (vis.MARGIN.left + vis.MARGIN.right);
        // vis.gHEIGHT8 = vis.height8 - (vis.MARGIN.top + vis.MARGIN.bottom);
        // vis.svg8.attr('width', vis.width8).attr('height', vis.height8);
        // vis.g8
        //     .attr('width', vis.gWIDTH8)
        //     .attr('height', vis.gHEIGHT8)
        //     .attr('transform', 'translate(' + vis.MARGIN.left + ',' + vis.MARGIN.top + ')');
        ...
        if (data[2].value && data[2].indexValue) {
            // JOIN
            ...
            // 3 year timeframe error message
            const output = `Frund originated in ${new Date(vis.data.inception).getFullYear()}. No returns are available for time frame.`;
            vis.g3.append('text').text(output);
            const opacitySetting = vis.draws === 1 ? 0 : 1;

            document.getElementById(`${vis.el}-3`).innerHTML = ` 
                <div class='bar-chart-output-text' id='bc-output-text-${vis.section}-0' style='opacity: ${opacitySetting}'>
                    ${output}
                </div
            `;
            document.getElementById(`bc-output-text-${vis.section}-0`).style.top = y(0) - document.getElementById(`bc-output-text-${vis.section}-0`).clientHeight + 10 + 'px';
            document.getElementById(`bc-output-text-${vis.section}-0`).style.opacity = 1;
        }
        ...
        if (data[3].value && data[3].indexValue) {
            ...
            vis.g4.append('text').text(output);
            const opacitySetting = vis.draws === 1 ? 0 : 1;
    ...
    setText() {
        ...
        function setElements(group = '') {
            ...
            // FOOTNOTES IN THE HEADER AND YTD<SUP>
            if (isMopBond) {
                document.getElementById('bond-q-end-footnote' + group).innerText = '3, 4'
            } else {
                document.getElementById('bond-q-end-footnote' + group).innerText = '3'
            }

            // console.log('barData', data)
            // console.log(vis.expense)
            // UPDATE VALUES
            ...
            data.forEach((d, i) => {
                ...
                if (document.getElementById(`bar-chart-val-${num}-${vis.section}${group}`)) {
                    if (i === data.length - 1) {
                        document.getElementById(`bar-chart-val-${num}-${vis.section}${group}`).innerText = enDash(vis.expense.toFixed(2)) + '%';
